        ...
        public override void AI()
        {
            ...
            if (state == 0)
            {
                ...
                else if (Projectile.ai[0] >= 15)
                {
                    ...
                    foreach (var proj in Main.projectile)
                    {
                        ...
                        {
                            if (collision == null || proj.type == ModContent.ProjectileType<UKCoin>() && collision.type != ModContent.ProjectileType<UKCoin>() && proj.damage > 0 || Microsoft.Xna.Framework.Vector2.Distance(Main.player[Main.myPlayer].position, proj.position) <
            ...
            else if (state == 1)
            {
                ...
                if (cointarget == null)
                {
                    ...
                    {
                        if (npc.life > 0 && npc.damage > 0 && (target == null || Main.player[Main.myPlayer].Distance(npc.position) < Main.player[Main.myPlayer].Distance(target.position)))
                        {
                            target = npc;
                        }
                    }
